...
test('buildBatchKey stays unique for different source keys that sanitize similarly', () => {
  ...
  const a = __test.buildBatchKey('logs/20260422/a:b.log.gz', 0, runId);
  const b = __test.buildBatchKey('logs/20260422/a/b.log.gz', 0, runId);

  assert.notEqual(a, b);
  assert.match(a, /processed-backfill\/.+-[a-f0-9]{32}-0\.txt$/);
  assert.match(b, /processed-backfill\/.+-[a-f0-9]{32}-0\.txt$/);
});
...
test('cleanup helpers treat sent .txt and .queued artifacts as non-pending', async () => {
  ...
  const pendingTxt = await __test.isPendingCleanupArtifact({ RAW_BUCKET }, 'processed-backfill/run-1/batch-00000001.txt');
  const pendingQueued = await __test.isPendingCleanupArtifact({ RAW_BUCKET }, 'processed-backfill/run-1/batch-00000001.txt.queued');
  const pendingOther = await __test.isPendingCleanupArtifact({ RAW_BUCKET }, 'processed-backfill/run-1/batch-00000002.txt');
